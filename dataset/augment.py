...
def random_rotation(img, boxes, degree=10):
    """
    :param img: PIL Image
    :param boxes:  numpy array [N, 4] (x1,y1,x2,y2)
    :return:
    """
    ...
    for i in range(boxes.shape[0]):
        ...
        x2, y2 = xmax, ymin
        x3, y3 = xmax, ymax
        z = torch.FloatTensor([[x0,y0], [x1,y1], [x2,y2], [x3,y3]])
        tp = torch.zeros_like(z)
        tp[:, 0] = (z[:, 0] - rx0) * math.cos(a) - (z[:, 1] - ry0) * math.sin(a) + rx0
        tp[:, 1] = (z[:, 0] - rx0) * math.sin(a) + (z[:, 1] - ry0) * math.cos(a) + ry0
        xmin, ymin = torch.min(tp, dim= 0)[0]
        xmax, ymax = torch.max(tp, dim= 0)[0]
        new_boxes[i,:] = torch.FloatTensor([xmin, ymin, xmax, ymax])
